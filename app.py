...
def display_zapier_data(company_id, df_zapier):
    zapier_data = df_zapier[df_zapier['ID'] == company_id]
    
    if not zapier_data.empty:
        st.subheader("Zapier Integrations")
        for _, row in zapier_data.iterrows():
            with st.container():
                col1, col2 = st.columns([1, 3])
                with col1:
                    st.image("https://cdn.zapier.com/zapier/images/logo-small.png", width=100)
                with col2:
                    st.markdown(f"### {row['Name']}")
                    st.markdown(f"**Category:** {row['Category']}")
                    st.markdown(f"**API:** {row['API Name']} ({row['API Type']})")
                st.markdown(f"**Description:** {row['Descroption']}")
                st.markdown(f"[View on Zapier]({row['Link to Zapier']})")
                st.markdown("---")
    else:
        st.info("No Zapier integrations available for this company.")
...
def main():
    ...
    st.image("geb-logo.png", width=80, use_column_width=False)  # Replace 'path_to_your_logo_image' with the path to your logo image
